...
def vendedor_required(func):
    ...
    def decorated_view(*args, **kwargs):
        if session.get('user_role') != 'vendedor':
            flash('Acceso no autorizado. Debes ser vendedor para ver esta página.', 'error')
...
def pagina_de_inicio():
    if 'logged_in' in session and session['logged_in']:
        ...
        return render_template('index.html', username=username, ultimas_ventas=ultimas_ventas, cantidad_total_ventas=cantidad_total_ventas, cantidad_total_usuarios=cantidad_total_usuarios, cantidad_total_articulos=cantidad_total_articulos, informacion_empresa=informacion_empresa)
    else:
...
def pagina_de_editar():
    ...
    datos = obtener_datos_de_articulo()
    if datos is not None:
        return render_template('editar.html', articulos=datos, username=session.get('usuario_actual'))
...
def mostrar_auditoria_ventas():
    ...
    return render_template('auditoria_inicio.html', registros_inicios_sesion=registros_inicios_sesion, username=session.get('usuario_actual'))
...
def mostrar_auditoria_inicio():
    ...
    return render_template('auditoria_inicio.html', username=session.get('usuario_actual'))
...
def mostrar_formulario_modificar_folios():
    ...
    return render_template('modificar_folios.html', username=session.get('usuario_actual'))
...
def mostrar_formulario_registro_usuario():
    ...
    return render_template('registro_usuario.html', username=session.get('usuario_actual'))
...
def ver_datos():
    ...
    if datos is not None:
        return render_template('ver_datos.html', articulos=datos, username=session.get('usuario_actual'))  # Asegúrate de usar 'articulos'
    else:
...
def registro_producto():
    if request.method == 'POST':
        ...
        try:
            ...
            conn = sqlite3.connect('BDD.db')  # Reemplaza con el nombre de tu base de datos
            cursor = conn.cursor()
            ...
            if producto_existente:
                flash(f'El producto con ID {id_producto} ya existe en la base de datos.', 'error')
            else:
                # Registra el nuevo producto en la base de datos
                cursor.execute('INSERT INTO articulo (id, nombre, descripcion, cantidad, precio, ubicacion,cantidad_maxima,cantidad_minima) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
                (id_producto, nombre, descripcion, cantidad, precio, ubicacion,cantidad_maxima,cantidad_minima))
                conn.commit()
                ...
                with open(nombre_archivo_csv, mode='a', newline='') as archivo_csv:
                    ...
                    escritor_csv.writerow([id_producto, nombre, cantidad, precio,cantidad_maxima, cantidad_minima, fecha_hora_actual])
    ...
    with open('users.txt', 'r') as file:
        for line in file:
            ...
            if len(parts) == 3:
                ...
                print(f'User: {stored_username}, Password: {stored_password}, Role: {user_role}')
    ...
    if request.method == 'POST':
        ...
        if user_role == 'administrador':
            ...
            with open('iniciosesion.txt', 'a') as file:
                file.write(f'Usuario: {username}, Fecha de inicio de sesión: {fecha_inicio_sesion}\n')
    ...
    if request.method == 'POST':
        ...
        if existing_article:
            ...
            if update_values:
                ...
                cursor.execute(f'UPDATE articulo SET {set_clause} WHERE id = ?', (*update_values.values(), id))
                conn.commit()
...
def ver_datos_editar():
    datos = obtener_datos_de_articulo()
    if datos is not None:
        return render_template('editar.html', articulos=datos)  # Asegúrate de usar 'articulos'
    else:
        return "Error al obtener datos de la base de datos."
...
def auditoria_ventas():
    ...
    if fecha_inicio and fecha_fin:
        datos_auditoria_filtrados = filtrar_por_fechas(datos_auditoria, fecha_inicio, fecha_fin)
    else:
        datos_auditoria_filtrados = datos_auditoria

    return render_template('auditoria.html', datos_auditoria=datos_auditoria_filtrados, username=session.get('usuario_actual'))
            ...
            for line in file:
                ...
                if len(parts) == 2:
                    ...
                    registros_inicios_sesion.append({
                        ...
                        'fecha_hora': fecha_hora.replace('Fecha de inicio de sesión: ', '')
                    })
    except FileNotFoundError:
...
def obtener_ultimo_folio_utilizado():
    try:
        ...
        return 0  # Si no existe el archivo, asumimos que no se ha utilizado ningún folio
...
def pagina_de_ventas():
    ...
    return render_template('ventas.html', total_precio=total_precio, detalles_productos=detalles_productos)
...
def agregar_venta():
    if request.method == 'POST':
        ...
        conn = sqlite3.connect('BDD.db')
        cursor = conn.cursor()
        cursor.execute('SELECT id, nombre, cantidad FROM articulo WHERE id = ?', (id_producto,))
        ...
        if articulo:
            ...
            if cantidad_venta <= articulo[2]:
                ...
                flash(f'Se ha agregado {cantidad_venta} unidades de {articulo[1]} a la lista de venta.', 'success')
            else:
                flash(f'La cantidad solicitada de {articulo[1]} no está disponible.', 'error')
        else:
            flash(f'El artículo con ID {id_producto} no fue encontrado en la base de datos.', 'error')
...
def guardar_registro_de_venta(id_venta, tipo_venta, rut_cliente, nombre_producto, total_venta, fecha_venta, folio):
    try:
        ...
        with open(archivo_csv, mode='a', newline='') as archivo:
            ...
            escritor_csv.writerow([id_venta, tipo_venta,rut_cliente, nombre_producto, total_venta, fecha_venta, folio])
...
def completar_venta():
    if request.method == 'POST':
        ...
        rut_cliente = request.form.get('rut_cliente')  # Obtén el RUT del cliente (opcional)       
        ...
        for item in lista_temporal_venta:
            ...
            guardar_registro_de_venta(id_venta, tipo_venta, rut_cliente, nombre_producto, total_venta, fecha_venta, folio)
...
def informes():
    ...
    return render_template('informes.html', img_path='ventas_globales.png', tablas=tablas_html, img_base64=img_base64, username=session.get('usuario_actual'))
...
def generar_tablas_html(ventas_por_tipo):
    ...
    for tipo_venta, ventas in ventas_por_tipo.items():
        ...
        for venta in ventas:
            id_venta, tipo_venta, rut_cliente, nombre_producto, total_venta, fecha_venta, folio = venta
        ...
        tabla_html += f'<div class="total-recuadro">Recaudación Total: {suma_total}</div>'
...
def generar_informe_pdf():
    ...
    data = [['ID Venta', 'Tipo Venta', 'RUT Cliente', 'Nombre Producto', 'Total Venta', 'Fecha Venta', 'Folio']]
...
def mostrar_informacion_empresa():
    ...
    return render_template('informacion_empresa.html', informacion_empresa=informacion_empresa, username=session.get('usuario_actual'))
...
def modificar_informacion_empresa():
    if request.method == 'POST':
        ...
        if 'logo' in request.files:
            ...
            if logo.filename != '':
                ...
                if nombre_imagen_anterior:
                    imagen_anterior_path = os.path.join('static', nombre_imagen_anterior)
        ...
        with open('empresa.csv', mode='w', newline='') as archivo_csv:
            campos = ['Nombre', 'RUT', 'Dirección', 'Número de Contacto', 'Correo Electrónico', 'Logo', 'Giro']
            escritor_csv = csv.DictWriter(archivo_csv, fieldnames=campos)
            escritor_csv.writeheader()
...
def registro_usuario():
    if request.method == 'POST':
        username = request.form['username']
        password = request.form['password']
        role = request.form['role']  # Acceder al valor seleccionado del campo de selección
...
#    pdf.cell(200, 10, txt="Datos Empresa", ln=True, align='C')
    ...
    pdf.cell(200, 10, txt=f"Número de Contacto: {datos_venta['Número de Contacto']}", ln=True, align='C')
    ...
    pdf.cell(200, 10, txt=f"Número de Contacto: {datos_venta['Número de Contacto']}", ln=True)
    pdf.cell(200, 10, txt="--------------------------------------------------------------------------------------------------------------------------------------", ln=True)  # Línea en blanco
    ...
    pdf.cell(200, 10, txt="________________________________________________________________________________", ln=True)  # Línea en blanco
...
def mostrar_archivos_pdf():
    ...
    archivos_pdf = obtener_lista_archivos_pdf(directorio_pdf)  # Pasa el directorio como argumento
...
def cotizar():
    ...
    archivos_pdf = [archivo for archivo in os.listdir(carpeta_cotizaciones) if archivo.endswith('.pdf')]
    ...
    if request.method == 'POST':
        ...
        conn = sqlite3.connect('BDD.db')
        cursor = conn.cursor()
        cursor.execute('SELECT id, nombre, precio, cantidad FROM articulo WHERE id = ?', (id_producto,))
        producto = cursor.fetchone()
        ...
        if producto:
            ...
            if cantidad_cotizada <= cantidad:
                ...
                flash(f'Se ha agregado {cantidad_cotizada} unidades de {nombre_producto} a la cotización.', 'success')
            else:
                flash(f'La cantidad solicitada de {nombre_producto} no está disponible.', 'error')
        else:
            flash(f'El producto con ID {id_producto} no fue encontrado en la base de datos.', 'error')
    ...
    return render_template('cotizar.html', productos_disponibles=productos_disponibles, lista_temporal_cotizacion=lista_temporal_cotizacion, archivos_pdf=archivos_pdf)
...
def ingresar_datos():
    if request.method == 'POST':
        ...
        datos_cotizacion = [cotizacion_id, fecha_cotizacion, nombre_cliente, rut_cliente, observaciones, descuento]
        ...
        productos_cotizados = []
        precio_total_cotizacion = 0  # Variable para calcular el precio total de la cotización
        ...
        with open('cotizacion.csv', mode='a', newline='') as file:
            ...
            if file.tell() == 0:
                writer.writerow(['ID', 'Fecha Cotización', 'Nombre Cliente', 'RUT Cliente', 'Observaciones', 'Descuento', 'Precio Total', 'Productos Cotizados'])
        ...
        generar_pdf_cotizacion(cotizacion_id, fecha_cotizacion, nombre_cliente, rut_cliente, observaciones, descuento, productos_cotizados)
...
def guardar_cotizacion():
    ...
    with open('cotizacion.csv', mode='w', newline='') as file:
        ...
        if not file.readline():
            writer.writerow(['ID', 'Fecha Cotización', 'Nombre Cliente', 'RUT Cliente', 'Observaciones', 'Descuento', 'Productos Cotizados', 'Precio Total'])
        ...
        for i, item in enumerate(lista_temporal_cotizacion, start=1):
            ...
            writer.writerow([i, fecha_cotizacion, nombre_cliente, rut_cliente, observaciones, descuento, item['nombre_producto'], item['cantidad_cotizada'], item['precio_producto'], precio_total])
...
def generar_pdf_cotizacion(cotizacion_id, fecha, cliente, rut, observaciones, descuento, productos):
    pdf = FPDF()
    pdf.add_page()
    pdf.set_font("Arial", size=12)
